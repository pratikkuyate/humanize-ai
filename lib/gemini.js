...
import {
  ...
  buildMultilingualStage2Prompt,
  buildMultilingualCritiquePrompt,
  buildMultilingualStage3Prompt,
} from "./prompts.js";
 ...
 *
 * English uses the hand-tuned prompts; other supported languages use the
 * multilingual builders with per-language rules from lib/languageRules.js.
 *
 * @param {string} rawText
 * @param {string} [language] "en" (default) or a key of languageRules
 * @returns {Promise<{ humanizedText: string; metadata: import('./types.js').Stage1Metadata }>}
 */
export async function humanizeText(rawText, language = "en") {
  const preprocessor = new Stage1Preprocessor();
  /** @type {Stage1Result} */
  const { cleanedText, metadata } = preprocessor.process(rawText, language);
  ...
  const stage2Text = await callGemini(client, buildMultilingualStage2Prompt(cleanedText, metadata, rules));
  const critiqueText = await callGemini(client, buildMultilingualCritiquePrompt(cleanedText, stage2Text, rules));
  const finalText = await callGemini(client, buildMultilingualStage3Prompt(critiqueText, rules));
