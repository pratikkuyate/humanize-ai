...
export class Stage1Preprocessor {
   ...
   * @param {string} [language] BCP-47-ish code ("en", "es", "fr", …). The
   *   deterministic transforms (clichés, vocabulary, contractions) and the
   *   pattern counts (passive, transitions, hedging) are English regexes, so
   *   they only run for English. The structural analysis (burstiness,
   *   sentence lengths, repeated starts, frequent words) is language-aware.
   * @returns {Stage1Result}
   */
  process(rawText, language = "en") {
    ...
    const metadata = this._analyze(text, sentences, language);
   ...
   */
  _analyze(fullText, sentences, language = "en") {
    ...
    return {
      ...
      frequentWords: this._findFrequentWords(sentences, language),
      // The three pattern counts below are English-regex based; they'd report
      // misleading zeros-or-noise for other languages, so we skip them there
      // (the multilingual prompts carry their own per-language tables instead).
      passiveSentenceCount: isEnglish ? this._countPassiveSentences(sentences) : 0,
      formalTransitionsFound: isEnglish ? this._findFormalTransitions(fullText) : [],
      hedgingPhraseCount: isEnglish ? this._countHedgingPhrases(fullText) : 0,
    };
  }
  ...
  _findRepeatedStarts(sentences) {
    ...
    for (const s of sentences) {
      // \p{L} instead of [a-z] so accented/non-ASCII first words survive;
      // identical behavior for plain-English input (text is lowercased first).
      const word = s.split(/\s+/)[0]?.toLowerCase().replace(/[^\p{L}]/gu, "");
   ...
   * @param {string} [language]
   */
  _findFrequentWords(sentences, language = "en") {
    /** @type {Map<string, number>} */
    const counts = new Map();
    const stopWords =
      language === "en"
        ? STOP_WORDS
        : new Set(languageRules[language]?.stopWords ?? []);
    const tokens =
      sentences.join(" ").toLowerCase().match(/[\p{L}]{3,}/gu) ?? [];
    for (const t of tokens) {
      if (!stopWords.has(t)) counts.set(t, (counts.get(t) ?? 0) + 1);
    }
    return [...counts.entries()]
      .filter(([, n]) => n > 2)
      .sort(([, a], [, b]) => b - a)
